...
def ks903imusicsongs():
    ...
    def change_slider_color(volume_value):
        ...
        # ttk.Styleを使ってスライダーの色を変更
        style = ttk.Style()
        style.configure("TScale",
                        slidercolor=color,       # スライダーのつまみ部分の色
                        )
    # # スライダーの色を音量に基づいて変更
    # def change_slider_color(volume_value):
    #     # 音量に応じた色を選択
    ...
    # 顔認証ロック状態を管理する変数
    face_lock_enabled = BooleanVar(value=False)  # 初期状態は無効

    # 顔認証トグル関数
    def fase_track():
        if face_lock_enabled.get():
            print("Face Recognition Security Lock: Enabled")
            # 顔認証有効化の処理を追加
        else:
            print("Face Recognition Security Lock: Disabled")
            # 顔認証無効化の処理を追加
    # メニュー項目の設定
    ...
    #     # Ctrl + Shift + Fでトグル
    #     face_lock_enabled.set(not face_lock_enabled.get())
    #     toggle_face_lock()
