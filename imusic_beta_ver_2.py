...
def change_slider_color(volume_value):
    ...
    style.configure("TScale",
                    troughcolor=color,       # スライダーの進行バー色
                    background=color,        # スライダーのバー色
                    sliderlength=20,          # スライダーの長さ
                    )
...
# スライダーの設定
volume_slider = ttk.Scale(volume_frame, from_=1, to=0, orient=VERTICAL, command=on_slider_motion, length=165)#command=volume, length=165)
